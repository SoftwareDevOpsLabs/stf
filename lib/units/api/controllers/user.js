...
// @chenhao 给用户设置标签
function setUserTags(req, res){
    var email = req.body.email
    var tags = req.body.tags
    dbapi.setUserTags(email, tags)
      .then(function(cursor) {
        res.json({
          success: true
        })
      ...
      .catch(function(err) {
        log.error('Failed to load data "%s": ', err.stack)
        res.status(500).json({
          success: false
        })
      })
...
// @chenhao 给server设置标签
function setServerTags(req, res){
  var name = req.body.name
  var note = req.body.note
  var tags = req.body.tags
  dbapi.setServerTags(name, note, tags)
    .then(function(cursor) {
      res.json({
        success: true
      })
