...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, router, push, group) {
    var log = logger.createLogger('device:plugins:graylog-agent')
    ...
    plugin.start = function(filters) {
      return group.get()
        .then(function(group) {
          return plugin.stop()
            .then(function() {
              log.info('Starting graylog agent')
    ...
    plugin.stop = Promise.method(function() {
      if (plugin.isRunning()) {
        log.info('Stopping graylog agent')
        activeLogcat.end()
        activeLogcat = null
      }
    })
    ...
    router
      ...
      .on(wire.LogcatStopMessage, function(channel) {logcat.js
        var reply = wireutil.reply(options.serial)
        plugin.stop()
          .then(function() {
            push.send([
