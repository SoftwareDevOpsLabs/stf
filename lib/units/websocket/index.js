  ...
  io.on('connection', function (socket) {
	...
	// @HY 2017-06-09 send log to Graylog server
      ...
      .on(wire.TestStatusMessage, function (channel, message) {
        console.log("[HYDEBUG websocket]: +++++++++ TestStatusMessage ++++++++++") // DEBUG
        console.log("[HYDEBUG websocket]: channel ", channel) // DEBUG
        console.log("[HYDEBUG websocket]: message ", message) // DEBUG

        io.emit('testing.status', {'id': message.testID, 'status': message.status})
      })
      .handler()
    ...
    new Promise(function (resolve) {
      ...
      // Device note
        ...
        .on('startTest', function (channel, data) {
          ...
          var run_env = data.run_env || "device" // 测试工具执行环境： 设备 or 服务器 @HY 2017-06-18
          ...
          if (run_env === "device") {
            ...
            var payload = {
              ...
              ,testID: testID
              ,command: commands
              ,timeout: 10000  // TODO: add this in web @HY 2017-06-18
              ,statusCheckCommand: package
