      ...
      // Device note
          ...
          dbapi.saveTest(serial, tester)
            .then(function () {
              ...
              if (run_env === "device") {
                ...
                tester['pid'] = exec.pid
                dbapi.saveTest(serial, tester)  // update pid to DB after creation of new process
        ...
        .on('stopTest', function (channel, data) {
          ...
          if (run_env === "device") { // run on device
            ...
            log.info("send stop test message to Device")
            var responseChannel = 'tx.' + uuid.v4()
            joinChannel(responseChannel)  // TODO: should remove it after using
            push.send([
              deviceChannel
                ...
                return Promise.promisify(cursor.toArray, cursor)()
                  .then(function (list) {
                    list.forEach(function (obj) {
                      log.error("Child process's pid", obj.pid)
