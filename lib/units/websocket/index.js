    ...
    new Promise(function (resolve) {
      ...
      // Device note
        ...
        .on('startTest', function (channel, data) {
          ...
          exec = spawn(commands.shift(), commands)

          // @hy 2017-5=05-21 add an error event to handle "ENOENT" error from chile process
          // Timeing is very import, MUST place the "error" handler after spawn
          ...
          if (typeof(exec) === 'undefined' || typeof(exec.pid) === 'undefined')
            return
          // throw new Error({message: "failed to invoke spawn"})
        ...
        .on('testing.testcase.save', function (data) {
          if (group == "" || scenario == "") {
            throw new Error({success: false, message: "测试模板格式不正确！"})
          }
          ...
          // TODO: 2017-05-18 we should exclude duplicate case(same group and scenario)
          //                  also exclude empty group and scenario
