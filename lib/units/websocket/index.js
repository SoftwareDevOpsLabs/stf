  ...
  io.on('connection', function (socket) {
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.DeviceLogcatEntryMessage, function (channel, message) {
        ...
        log.info("++++++++++++++++++LOGCAT Entry MSG++++++++++++++") // @HY DEBUG
        log.info(message)  // @HY DEBUG

        // TODO: @HY 2017-05-30 write logcat into file logcat/serial.log
        var fs = require('fs')
        var filename = "./logcat/" + message.serial + ".log"
        log.info("++++++++++++++++++Write LOGCAT Entry MSG++++++++++++++") // @HY DEBUG
		...
		if (err) {
		   ...
		   var req = request.post(url, function (err, resp, body) {	
			   ...
			   log.error("ERROR: " + err)	
                           ...
                           log.error('Upload to "%s" failed: HTTP %d', url, res.statusCode)
		       } else {
                           try {
