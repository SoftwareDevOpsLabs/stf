...
module.exports = db.ensureConnectivity(function(options) {
  ...
  devDealer.on('message', wirerouter()
    ...
    .on(wire.TestStatusMessage, function(channel, message, data) {
      // dbapi.setDeviceReverseForwards(message.serial, message.forwards)
      console.error("Processor channel", channel.toString())
      console.error("Processor message", message)
      console.error("Processor data", data.toString())

      if (typeof message.status !== "undefined"
          && message.status !== "Testing") {

        dbapi.saveTest(message.serial, {'id': message.testID, 'status': message.status})
          .then(function() {
            // @HY 2017-6-20, kick group after closing testing
