...
//TODO @chenhao 存储测试的输出结果
dbapi.saveTest = function(serial, tester){
  return db.run(r.table('testing').get(tester.id).update({
     end: tester.endTime
    ...
    .then(function(stats) {
      if (stats.skipped) {
        return db.run(r.table('testing').insert({
          id: tester.id
          ,group: tester.groupId
          , name: tester.name
          , user: tester.user
          , serial: serial
          , start: tester.startTime
          , end: tester.endTime
          , status: tester.status
          , commands: tester.commands
          , pid: tester.pid
          , model: tester.model
...
// 某个用户的测试记录,
dbapi.loadTestingList = function(status){
  //return db.run(r.table('testing').filter({'status':status}).group('group'))
  if (status === 'Notrun'){ // @hy 2017-05-08
    return db.run(r.table('testing').filter(
      r.row('status').eq('Finish').
      or(r.row('status').eq('Stop')).
      or(r.row('status').eq('Failed')).  // @hy 2017-05-10 add a state: Failed
      or(r.row('status').eq('Timeout'))).orderBy('-start')
    )
  }else{
    return db.run(r.table('testing').filter({'status':status}).orderBy('-start'))
  }
}
...
dbapi.loadTestingReport = function(id){
  return db.run(r.table('reports').filter({test:id}).limit(200))
}

dbapi.loadTestingPieStat = function(name){
  return db.run(r.db('stf').table('testing').filter({'status':'Failed'}).group(name))
  //return db.run(r.db('stf').table('testing').group(name)) 测试所用
}

dbapi.loadTestingBarStat = function(name){
