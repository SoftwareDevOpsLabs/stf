...
// 某个用户的测试记录,
dbapi.loadTestingList = function(status, start, end, type){
  ...
  if (start==0){
    ...
    var query_filter = r.row('start').gt(start).and(r.row('start').lt(end)).and({'name':type})
    if (status === 'Notrun'){
      return db.run(r.table('testing').orderBy(r.desc('start')).filter(
        r.row('status').ne('Testing')
      ).filter(query_filter))
    }else{
      return db.run(r.table('testing').orderBy(r.desc('start')).filter({'status':status})).filter(query_filter)
    }
  }
}
...
dbapi.loadTestingReport = function(id){
  return db.run(r.table('reports').filter({test:id}).orderBy('time').limit(500))
}
...
dbapi.loadTestingPieStat = function(name, start, end, type){
  if (start==0){
    ...
    var query_filter = r.row('start').gt(start).and(r.row('end').gt(end)).and({'name':type})
    return db.run(r.db('stf').table('testing').filter({'status':'Failed'}).filter(query_filter).group(name))
  }
}
