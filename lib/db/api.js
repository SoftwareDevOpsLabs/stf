...
dbapi.loadTestingReport = function(id){
  return db.run(r.table('reports').filter({test:id}).limit(500).orderBy('time'))
}

dbapi.loadTestingPieStat = function(name, start, end, type){
  if (start==0){
    return db.run(r.db('stf').table('testing').filter({'status':'Stop'}).group(name))
  }else{
    var query_filter = r.row('start').gt(start).and(r.row('end').lt(end)).and({'name':type})
    return db.run(r.db('stf').table('testing').filter({'status':'Stop'}).filter(query_filter).group(name))
  }
}
